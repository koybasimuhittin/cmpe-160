...
public final class StdDraw
  ...
  public static void setCanvasSize(int canvasWidth, int canvasHeight) {
    if (canvasWidth <= 0) throw new IllegalArgumentException(
      "width must be positive"
    );
    if (canvasHeight <= 0) throw new IllegalArgumentException(
  ...
  private static void init() {
    ...
    frame = new JFrame();
    offscreenImage =
      new BufferedImage(2 * width, 2 * height, BufferedImage.TYPE_INT_ARGB);
    onscreenImage =
      new BufferedImage(2 * width, 2 * height, BufferedImage.TYPE_INT_ARGB);
  ...
  public static void setXscale(double min, double max) {
    ...
    validate(max, "max");
    double size = max - min;
    if (size == 0.0) throw new IllegalArgumentException(
  ...
  public static void point(double x, double y) {
    ...
    if (scaledPenRadius <= 1) pixel(x, y); else offscreen.fill(
  ...
  public static void filledCircle(double x, double y, double radius) {
    ...
    double ws = factorX(2 * radius);
    double hs = factorY(2 * radius);
    if (ws <= 1 && hs <= 1) pixel(x, y); else offscreen.fill(
      new Ellipse2D.Double(xs - ws / 2, ys - hs / 2, ws, hs)
    );
    draw();
  }
   ...
   * @throws IllegalArgumentException if any argument is either NaN or infinite
   */
  public static void filledEllipse(
    double x,
    double y,
    double semiMajorAxis,
    double semiMinorAxis
...
*/
  ...
  public static void picture(double x, double y, String filename) {
    ...
    if (ws < 0 || hs < 0) throw new IllegalArgumentException(
  ...
  public static void save(String filename) {
    ...
    if (filename.length() == 0) throw new IllegalArgumentException(
      "argument to save() is the empty string"
    );
    ...
    try {
      ...
      saveImage
        .createGraphics()
        .drawImage(onscreenImage, 0, 0, Color.WHITE, null);
